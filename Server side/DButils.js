...
var connectionConfig = {
  userName: "gonile",
  password: "gonI1992",
  server: "assignment3webdevs.database.windows.net",
...
exports.execQuery = function(query) {
  return new Promise(function(resolve, reject) {
    try {
      var ans = [];
      var properties = [];
      console.log(connectionConfig.userName + " " + connectionConfig.password);
      ...
      pool.acquire(function(err, connection) {
        ...
        dbReq.on("columnMetadata", function(columns) {
          columns.forEach(function(column) {
            if (column.colName != null) properties.push(column.colName);
          });
        });
        ...
        dbReq.on("requestCompleted", function() {
          console.log(
            "request Completed: " + dbReq.rowCount + " row(s) returned"
          );
          // console.log(ans);
          connection.release();
          resolve(ans);
